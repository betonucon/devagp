...
class FakturController extends BaseController
{
    public function faktur(Request $request)
    {
        ...
        $query=Viewfaktur::query();
        $get=$query->where('KD_Salesman',$akses->username)->where('KD_Customer',$request->KD_Customer)->orderBy('Tanggal','Desc')->paginate(20);
        $cek=$query->count();
        ...
        foreach($get as $o){
           $sub=[];
                $cl=[];
                $cl['toko'] =$o->Perusahaan;
                $cl['KD_Customer'] =$o->KD_Customer;
                $cl['total_barang'] = $o->total_barang;
    ...
    public function customer_faktur(Request $request)
    {
        ...
        $get=$query->where('KD_Salesman',$akses->username)->orderBy('Perusahaan','Desc')->paginate(20);
        $cek=$query->count();
        ...
        foreach($get as $o){
           $sub=[];
                ...
                $cl['nama_salesman'] = $o->nama_salesman;
